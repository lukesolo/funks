...
const {Lifted, Sync, ServiceCall, Or, OnFail, services, isPure} = require('./builder');
...
const build = comp => {
    ...
    if (isPure(top)) {
        if (Array.isArray(top)) {
            return buildArrayResult(top, builder);
        }
        if (top !== null && typeof top === 'object') {
            return buildObjectResult(top, builder);
        }
    }
